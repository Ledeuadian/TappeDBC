...
function OfflineQrModal({ card, onlineUrl, canvasId, filename, onClose }) {
  ...
  // True when the location string is raw lat/lng (geocoding failed) —
  // used to show a warning + retry option in the ready state.
  const [usedCoordsFallback, setUsedCoordsFallback] = useState(false)
  // Remember the coords so "Retry geocoding" can re-run just the lookup.
  ...
  const handleGenerate = async () => {
    ...
    setUsedCoordsFallback(!!coords && !where)
    setStamps({
      when: when.toLocaleString(),
      where: where || (coords ? `${coords.lat.toFixed(5)}, ${coords.lng.toFixed(5)}` : null),
    })
    setStep('ready')
  }

  /** Re-run just the geocoding (no new GPS read, no new timestamp) —
   *  useful when the first lookup was blocked/rate-limited. */
  const handleRetryGeocode = async () => {
    if (!lastCoords) return
    setStep('locating')
    const geo = await reverseGeocode(lastCoords.lat, lastCoords.lng)
    const where = geo.formatted || null
    setUsedCoordsFallback(!where)
    setStamps((s) => ({ ...s, where: where || s.where }))
    setStep('ready')
  }
              ...
              {usedCoordsFallback && (
                <div className="mt-1">
                  <p className="text-amber-600">
                    Couldn't name this place — showing coordinates instead.
                  </p>
                  <button
                    type="button"
                    onClick={handleRetryGeocode}
                    className="mt-1 rounded-full px-3 py-1 text-[10px] font-semibold border border-amber-300 text-amber-700 hover:bg-amber-50 active:scale-[0.98] transition"
                  >
